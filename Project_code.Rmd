...
# Ever Travelled Abroad and anual income 
#scatter plot 

```{r include=TRUE}

ggplot(data, aes(x = as.factor(EverTravelledAbroad), y = AnnualIncomeUSD ) ) +
  geom_boxplot() +
  labs(title = "Box Plot of Annual Income by Ever Travelled Abroad",
       x = "Ever Travelled Abroad",
       y = "Annual Income") + 
    scale_y_continuous(labels = scales::dollar_format(), breaks = seq(from = min(data$AnnualIncomeUSD), to = max(data$AnnualIncomeUSD), by = 2000)) +  # Formats the y-axis labels as dollars and sets breaks
  theme_light() +  
  theme( 
...
# Graph 4 - graduate or not and anual income 

```{r include=TRUE}

ggplot(data, aes(x = as.factor(GraduateOrNot), y = AnnualIncomeUSD,  ) ) +
  geom_boxplot(fill = "blue", color = "red") +
  labs(title = "Box Plot of Annual Income by Graduate or Not",
       x = "Graduate or Not",
       y = "Annual Income") + 
    scale_y_continuous(labels = scales::dollar_format(), breaks = seq(from = min(data$AnnualIncomeUSD), to = max(data$AnnualIncomeUSD), by = 2000)) +  # Formats the y-axis labels as dollars and sets breaks
  theme_light() +  
...
ggplot(data, aes(x = factor(Age))) +
  ...
  labs(title = "Frequency of Each Age", x = "Age", y = "Frequency")
  
  
```

```{r}
# Graph 5 - Density Plot of Age 
ggplot(data, aes(x = Age)) +
  geom_density(fill = "lightcoral", color = "black") +
  labs(title = "Density Plot of Age", x = "BMI", y = "Density")
  
      labs(title = "Count Distribution of Employment Type", x = "Employment Type", y = "Count")
```

```{r}
# Graph 6 - Density Plot of Income
ggplot(data, aes(x = AnnualIncome)) +
  geom_density(fill = "lightcoral", color = "black") +
  labs(title = "Density Plot of BMI", x = "AnnualIncome", y = "Density")
  
```

```{r}
# Graph 7 - Bar Plot of Employment Type
ggplot(data, aes(x = factor(Employment.Type))) +
